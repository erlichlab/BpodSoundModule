...
class SoundServiceSimple(object):
    ...
    def __init__(self,List_Sound,volumelist,socket=None,fs=48000):
        self.socket = socket
...
class SoundServiceComp(object):
    ...
    Stim = None
    socket = None
    def __init__(self,List_Sound,volumelist,Loop,socket,fs=48000):
        self.socket = socket
        sd.default.samplerate=fs
        sd.default.device=2
        sd.default.latency=['low','low']
    ...
    def StartSnd(self):
        ...
        # open serial thread
        self.SerialDect()
        #self.tser=threading.Thread(target=self.SerialDect,name='serialdetect')
        #self.tser.start()
